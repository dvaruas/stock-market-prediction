...
class AnnTrain(AnnStructure):
	...
	def prepareTrainingData(self, annPath):
		...
		normalizeMinRange = -1.0
		normalizeMaxRange = 1.0
		
		if parameters.functionType == 1:
			normalizeMinRange = 0.0
		else:
			pass
		...
		dataValues = [[val[0], [self.helperObject.minMaxNormalization(val[1][indx], maxValueInput[indx], minValueInput[indx], normalizeMaxRange, normalizeMinRange) for indx in range(outputLength)]] for val in dataValues]
	...
	def validatePhase(self):
		...
		for data in validateData:
			...
			outputReceived = self.annResultObject.getAnnResults(inputs, self)
			error = 0.0
			i = 0
			for x in outputReceived:
				error += (x - output[i]) ** 2
				i += 1
			errors.append(error)
		...
		if averageError < self.error:
			...
			fileWriter = open('data/error_graph_ann.csv', 'a')
		...
		# 3000 iterations given to find another minimum
		else:
...
class AnnValue(AnnStructure):
	...
	def getAnnResult(self, inputs, annObject = None):
		...
		if annObject != None:
			self.hiddenNodes = list(annObject.hiddenNodes)
			self.outputNodes = list(annObject.outputNodes)
		else:
			pass
